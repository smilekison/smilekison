    ...
    <ol ref={ref} className="relative mt-14">
      ...
      {timeline.map((item, i) => (
        <motion.li
          ...
          className="relative grid grid-cols-[auto_1fr] gap-x-6 pb-12 last:pb-0 sm:grid-cols-[4rem_auto_1fr] lg:grid-cols-[6.75rem_auto_1fr] lg:gap-x-8"
        >
          {/* Year sits in its own column from sm up; inline on the smallest screens */}
          <span className="type-data col-start-2 row-start-1 mb-2 text-signal sm:col-start-1 sm:row-start-1 sm:mb-0 sm:pt-1 sm:text-right">
            {item.year}
          </span>

          <span
            className="relative z-10 col-start-1 row-start-1 mt-1.5 h-2.5 w-2.5 shrink-0 border border-signal bg-ink sm:col-start-2 sm:mt-2"
            aria-hidden
          />
